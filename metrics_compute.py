...
def compute_metrics1(x_ori, x_adv, lpips_model, lpips_model2):
    img_ori = x_ori[0].mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
    img_adv = x_adv[0].mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
    psnr_value = psnr(img_ori, img_adv)
    ssim_value = ssim(img_ori, img_adv, channel_axis=2)
    lpips_alex = lpips_model(x_ori, x_adv).item()
    lpips_vgg = lpips_model2(x_ori, x_adv).item()
